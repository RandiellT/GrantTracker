...
const trackProposalModel= require('../models/trackProposalModel');
...
router.post('/post', async (req, res) => {
    const data = new trackProposalModel({
...
router.get('/getAll', async (req, res) => {
    try{
        const data = await trackProposalModel.find();
        res.json(data)
    }
    catch(error){
...
router.get('/get/:id', async (req, res) => {
    try{
        const data = await trackProposalModel.findById(req.params.id);
        res.json(data)
    }
    catch(error){
        res.status(500).json({message: error.message})
...
router.put('/update/:id', async (req, res) => {
    try {
        ...
        const result = await trackProposalModel.findByIdAndUpdate(
...
router.delete('/delete/:id', async (req, res) => {
    try {
        const id = req.params.id;
        const data = await trackProposalModel.findByIdAndDelete(id)
