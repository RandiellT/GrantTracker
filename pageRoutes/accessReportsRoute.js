...
router.post('/post', async (req, res) => {
    const data = new accessReportsModel({
        submittedReport: req.body.submittedReport,
...
router.get('/getAll', async (req, res) => {
    try{
        const data = await accessReportsModel.find();
        res.json(data)
    }
    catch(error){
...
router.get('/get/:id', async (req, res) => {
    try{
        const data = await accessReportsModel.findById(req.params.id);
        res.json(data)
    }
    catch(error){
        res.status(500).json({message: error.message})
...
router.patch('/update/:id', async (req, res) => {
    try {
        ...
        const result = await accessReportsModel.findByIdAndUpdate(
...
router.delete('/delete/:id', async (req, res) => {
    try {
        const id = req.params.id;
        const data = await accessReportsModel.findByIdAndDelete(id)
